...
class EnvRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            order0_price /= order0_num
            order1_price /= order1_num
            order_price = (order0_price + order1_price) / (order0_num + order1_num)
            order_price_total.append([order0_price, order1_price, order_price])
            print(f"The average price of Courier0's order is {order0_price} with {order0_num} orders, Courier1's is {order1_price} with {order1_num} orders, for all is {order_price}")
        ...
        plt.grid(True)
        plt.savefig('Train_average_price_of_order.png')
