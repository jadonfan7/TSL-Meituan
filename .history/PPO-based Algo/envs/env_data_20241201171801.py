    ...
    def step(self, first_time=0):
        ...
        while(self.current_index < self.order_data.shape[0] and self.order_data.iloc[self.current_index]['platform_order_time'] <= self.clock):
            ...
            if order_id not in self.orders_id and dt['is_courier_grabbed'] == 1 and dt['estimate_arrived_time'] - dt['platform_order_time'] > 0:
                ...
                prepare_time = dt['estimate_meal_prepare_time'] - dt['platform_order_time']
                estimate_arrived_time = dt['estimate_arrived_time'] - dt['platform_order_time']
                waiting_time = self.clock - dt['platform_order_time']
                
                order = Order(order_id, is_prebook, is_in_the_same_da_and_poi, order_create_time, pickup_point, dropoff_point, prepare_time, estimate_arrived_time, waiting_time)
                orders.append(order)
