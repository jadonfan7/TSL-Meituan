...
class Map:
    ...
    def step(self, first_time=0):
        ...
        while(self.current_index < self.order_data.shape[0] and self.order_data.iloc[self.current_index]['platform_order_time'] <= self.clock):
            ...
            if order_id not in self.orders_id and dt['is_courier_grabbed'] == 1 and dt['estimate_arrived_time'] - dt['order_push_time'] > 0:
                ...
                prepare_time = dt['estimate_meal_prepare_time'] - dt['order_push_time']
                estimate_arrived_time = dt['estimate_arrived_time'] - dt['order_push_time']
                
                order = Order(order_type, order_id, order_create_time, pickup_point, dropoff_point, prepare_time, estimate_arrived_time)
                orders.append(order)
    ...
    def _accept_or_reject(order, courier):
                
        reward = 0
        fairness = 0
        
        num_waybill = len(courier.waybill + courier.wait_to_pick)
        potential_overspeed_risk = 0
        rejection_history_count = 0
        is_in_the_same_da_and_poi = 0
        is_prebooked = 0
        pick_up_distance = 0
        drop_off_distance = 0
        ...
        X = (reward, fairness, num_waybill, potential_overspeed_risk, rejection_history_count, is_in_the_same_da_and_poi, is_prebooked, pick_up_distance, drop_off_distance, estimate_arrived_time, estimate_meal_prepare_time, order)
    ...
    def _greedy_allocation(self, orders):
        ...
        for order in orders:
            ...
            if nearest_courier.position == order.pick_up_point and self.clock >= order.prepare_time:  # picking up
    ...
    def _bipartite_allocation(self, orders):
        ...
        for order in orders:
            ...
            for courier in couriers:
                max_speed = self._cal_speed(order, courier)
        ...
        for order_index, courier_index in zip(row_ind, col_ind):
            ...
            if courier.position == order.pick_up_point and self.clock >= order.prepare_time:
    ...
    def _equitable_allocation(self, orders):
        ...
        for i, p in enumerate(orders):
            ...
            for courier in nearby_couriers:
                max_speed = self._cal_speed(p, courier)
                avg_reward = courier.reward / (self.clock - courier.start_time) if (self.clock - courier.appear_time) != 0 else courier.reward
