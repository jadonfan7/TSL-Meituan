    ...
    def _set_action(self, action, agent):
        ...
        for order in agent.waybill:
            if agent.position == order.drop_off_point:  # dropping off
                ...
                if self.map.clock - order.order_create_time > order.ETA:
                    if agent.courier_type == 0:
                        reward -= 300 * ((self.map.clock - order.pair_time) / (order.ETA - order.order_create_time) - 1)
                    else:
                        reward -= 200 * ((self.map.clock - order.pair_time) / (order.ETA - order.order_create_time) - 1)
                    order.is_late = 1
                else:
                    order.ETA_usage = (self.map.clock - order.pair_time) / (order.ETA - order.order_create_time)
                    if agent.courier_type == 0:
